...
class PluginManager
{
     ...
     * @param Tangram    $globalTangram
     * @param bool        $disablePlugins
     */
    public function __construct(IOInterface $io, Tangram $tangram, Tangram $globalTangram = null, $disablePlugins = false)
    {
        $this->io = $io;
        $this->tangram = $tangram;
        $this->globalTangram = $globalTangram;
    ...
    public function loadInstalledPlugins()
    {
        ...
        $repo = $this->tangram->getRepositoryManager()->getLocalRepository();
        $globalRepo = $this->globalTangram ? $this->globalTangram->getRepositoryManager()->getLocalRepository() : null;
        if ($repo) {
    ...
    public function registerPackage(PackageInterface $package, $failOnMissingClasses = false)
    {
        ...
        $localRepo = $this->tangram->getRepositoryManager()->getLocalRepository();
        $globalRepo = $this->globalTangram ? $this->globalTangram->getRepositoryManager()->getLocalRepository() : null;
        ...
        foreach ($classes as $class) {
            ...
            if ($oldInstallerPlugin) {
                $installer = new $class($this->io, $this->tangram);
                $this->tangram->getInstallationManager()->addInstaller($installer);
    ...
    public function addPlugin(PluginInterface $plugin)
    {
        ...
        $this->plugins[] = $plugin;
        $plugin->activate($this->tangram, $this->io);

        if ($plugin instanceof EventSubscriberInterface) {
            $this->tangram->getEventDispatcher()->addSubscriber($plugin);
        }
    }
    ...
    {
        if (!$global) {
            return $this->tangram->getInstallationManager()->getInstallPath($package);
        }

        return $this->globalTangram->getInstallationManager()->getInstallPath($package);
    }
