...
class LibraryInstaller implements InstallerInterface, BinaryPresenceInterface
{
     ...
     */
    public function __construct(IOInterface $io, Tangram $tangram, $type = 'library', Filesystem $filesystem = null, BinaryInstaller $binaryInstaller = null)
    {
        $this->tangram = $tangram;
        $this->downloadManager = $tangram->getDownloadManager();
        $this->io = $io;
        $this->type = $type;

        $this->filesystem = $filesystem ?: new Filesystem();
        $this->vendorDir = rtrim($tangram->getConfig()->get('vendor-dir'), '/');
        $this->binaryInstaller = $binaryInstaller ?: new BinaryInstaller($this->io, rtrim($tangram->getConfig()->get('bin-dir'), '/'), $tangram->getConfig()->get('bin-compat'), $this->filesystem);
    }
