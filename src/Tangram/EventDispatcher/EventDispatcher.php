...
class EventDispatcher
{
     ...
     * Constructor.
     *
     * @param Tangram        $tangram The composer instance
     * @param IOInterface     $io       The IOInterface instance
     * @param ProcessExecutor $process
     */
    public function __construct(Tangram $tangram, IOInterface $io, ProcessExecutor $process = null)
    {
    ...
    {
        return $this->doDispatch(new Script\Event($eventName, $this->tangram, $this->io, $devMode, $additionalArgs, $flags));
    }
    ...
    {
        return $this->doDispatch(new PackageEvent($eventName, $this->tangram, $this->io, $devMode, $policy, $pool, $installedRepo, $request, $operations, $operation));
    }
    ...
    {
        return $this->doDispatch(new InstallerEvent($eventName, $this->tangram, $this->io, $devMode, $policy, $pool, $installedRepo, $request, $operations));
    }
    ...
    protected function doDispatch(Event $event)
    {
        ...
        $binDir = $this->tangram->getConfig()->get('bin-dir');
        if (is_dir($binDir)) {
        ...
        foreach ($listeners as $callable) {
            if (!is_string($callable) && is_callable($callable)) {
                ...
                if (substr($callable, 0, 10) === '@composer ') {
                    $exec = $this->getPhpExecCommand() . ' ' . ProcessExecutor::escape(getenv('TANGRAM_BINARY')) . substr($callable, 9);
                ...
                $possibleLocalBinaries = $this->tangram->getPackage()->getBinaries();
    ...
    {
        $package = $this->tangram->getPackage();
        ...
        $generator = $this->tangram->getAutoloadGenerator();
        if ($event instanceof ScriptEvent) {
            $generator->setDevMode($event->isDevMode());
        }

        $packages = $this->tangram->getRepositoryManager()->getLocalRepository()->getCanonicalPackages();
        $packageMap = $generator->buildPackageMap($this->tangram->getInstallationManager(), $package, $packages);
